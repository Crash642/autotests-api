...
def assert_get_user_response(get_user_response : GetUserResponseSchema, create_user_response: CreateUserResponseSchema):
    """
    Проверяет, что ответ на получение пользователя соответствует данным созданного пользователя.

    :param get_user_response: Ответ API с данными пользователя при получении.
